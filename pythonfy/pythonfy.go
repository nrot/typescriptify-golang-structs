...
func New() *Pythony {
	...
	kinds[reflect.Bool] = "bool"
	kinds[reflect.Interface] = "Any"

	kinds[reflect.Int] = "int"
	kinds[reflect.Int8] = "int"
	kinds[reflect.Int16] = "int"
	kinds[reflect.Int32] = "int"
	kinds[reflect.Int64] = "int"
	kinds[reflect.Uint] = "int"
	kinds[reflect.Uint8] = "int"
	kinds[reflect.Uint16] = "int"
	kinds[reflect.Uint32] = "int"
	kinds[reflect.Uint64] = "int"
	kinds[reflect.Float32] = "float"
	kinds[reflect.Float64] = "float"

	kinds[reflect.String] = "str"
	...
	result.DefaultImport = "import json\nfrom typing import *\n"
...
func (ts Pythony) logf(depth int, s string, args ...interface{}) {
...
func (t *Pythony) ManageType(fld interface{}, opts TypeOptions) *Pythony {
	var typ reflect.Type
	switch t := fld.(type) {
	case reflect.Type:
		typ = t
...
func (t *Pythony) WithCreateFromMethod(b bool) *Pythony {
	t.CreateFromMethod = b
	return t
}

func (t *Pythony) WithInterface(b bool) *Pythony {
	t.CreateInterface = b
	return t
}

func (t *Pythony) WithConstructor(b bool) *Pythony {
	t.CreateConstructor = b
	return t
}

func (t *Pythony) WithIndent(i string) *Pythony {
	t.Indent = i
	return t
}

func (t *Pythony) WithBackupDir(b string) *Pythony {
	t.BackupDir = b
	return t
}

func (t *Pythony) WithPrefix(p string) *Pythony {
	t.Prefix = p
	return t
}

func (t *Pythony) WithSuffix(s string) *Pythony {
	t.Suffix = s
	return t
}

func (t *Pythony) Add(obj interface{}) *Pythony {
	switch ty := obj.(type) {
	case StructType:
...
func (t *Pythony) AddType(typeOf reflect.Type) *Pythony {
...
func (t *typeScriptClassBuilder) AddMapField(fieldName string, field reflect.StructField) {
	...
	if valueType.Kind() == reflect.Array || valueType.Kind() == reflect.Slice {
		valueTypeName = valueType.Elem().Name() + "list"
	}
	if valueType.Kind() == reflect.Ptr {
		valueTypeName = valueType.Elem().Name()
	}
	strippedFieldName := strings.ReplaceAll(fieldName, "Optional", "")
	...
	if valueType.Kind() == reflect.Struct {
		...
		t.constructorBody = append(t.constructorBody, fmt.Sprintf("%s%sself.%s = source[\"%s\"];", t.indent, t.indent, strippedFieldName, strippedFieldName))
	}
}

func (t *Pythony) AddEnum(values interface{}) *Pythony {
	if t.enums == nil {
...
func (t *Pythony) AddEnumValues(typeOf reflect.Type, values interface{}) *Pythony {
	t.AddEnum(values)
	return t
}

func (t *Pythony) Convert(customCode map[string]string) (string, error) {
...
func (t Pythony) backup(fileName string) error {
...
func (t Pythony) ConvertToFile(fileName string) error {
	...
	if _, err := f.WriteString(t.DefaultImport); err != nil {
		return err
	}
...
func (t *Pythony) convertEnum(depth int, typeOf reflect.Type, elements []enumElement) (string, error) {
...
func (t *Pythony) getFieldOptions(structType reflect.Type, field reflect.StructField) TypeOptions {
	// By default use options defined by tags:
	opts := TypeOptions{PYTransform: field.Tag.Get(pyTransformTag), PYType: field.Tag.Get(pyType)}
	...
	for _, o := range overrides {
		if o.PYTransform != "" {
			opts.PYTransform = o.PYTransform
		}
		if o.PYType != "" {
			opts.PYType = o.PYType
		}
	}
...
func (t *Pythony) getJSONFieldName(field reflect.StructField, isPtr bool) string {
	jsonFieldName := ""
...
func (t *Pythony) convertType(depth int, typeOf reflect.Type, customCode map[string]string) (string, error) {
	if _, found := t.alreadyConverted[typeOf]; found { // Already converted
		return "", nil
	}
	...
	if !t.CreateInterface {
		...
		needsConvertValue := strings.Contains(constructorBody, "self.convertValues")
		if t.CreateFromMethod {
			result += fmt.Sprintf("\n%sdef createFrom(self, source: Any = {}): \n", t.Indent)
			result += fmt.Sprintf("%s%sreturn %s(source)\n", t.Indent, t.Indent, entityName)
			result += fmt.Sprintf("%s\n", t.Indent)
		}
		if t.CreateConstructor {
			result += fmt.Sprintf("\n%sdef __init__(self, source: Any = {}): \n", t.Indent)
			result += t.Indent + t.Indent + "if isinstance(source, str):\n"
			result += t.Indent + t.Indent + t.Indent + "source = json.loads(source)\n"
			result += constructorBody + "\n"
			result += fmt.Sprintf("%s\n", t.Indent)
		}
		if needsConvertValue && (t.CreateConstructor || t.CreateFromMethod) {
			result += "\n" + indentLines(strings.ReplaceAll(strings.ReplaceAll(pyConvertValuesFunc, "{ident}", t.Indent), t.Indent, t.Indent), 1, t.Indent) + "\n"
		}
	}
...
func (t *Pythony) AddImport(i string) {
	for _, cimport := range t.customImports {
		if cimport == i {
			return
		}
...
func (t *typeScriptClassBuilder) AddSimpleArrayField(fieldName string, field reflect.StructField, arrayDepth int, opts TypeOptions) error {
	...
	if len(fieldName) > 0 {
		...
		if len(opts.PYType) > 0 {
			t.addField(fieldName, opts.PYType)
			t.addInitializerFieldLine(strippedFieldName, fmt.Sprintf("source[\"%s\"]", strippedFieldName))
			return nil
		} else if len(typeScriptType) > 0 {
			t.addField(fieldName, strings.Repeat("list[", arrayDepth)+typeScriptType+strings.Repeat("]", arrayDepth))
			t.addInitializerFieldLine(strippedFieldName, fmt.Sprintf("source[\"%s\"]", strippedFieldName))
			return nil
		}
	}
...
func (t *typeScriptClassBuilder) AddSimpleField(fieldName string, field reflect.StructField, opts TypeOptions) error {
	...
	if len(typeScriptType) > 0 && len(fieldName) > 0 {
		...
		if opts.PYTransform == "" {
			...
			expression := strings.Replace(opts.PYTransform, "__VALUE__", val, -1)
...
func (t *typeScriptClassBuilder) addInitializerFieldLine(fld, initializer string) {
	...
	t.constructorBody = append(t.constructorBody, fmt.Sprint(t.indent, t.indent, "self.", fld, " = ", initializer, ""))
}
