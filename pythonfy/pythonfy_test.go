...
func testConverter(t *testing.T, converter *Pythony, strictMode bool, desiredResult string, tsExpressionAndDesiredResults []string) {
...
func TestDateWithoutTags(t *testing.T) {
	...
	converter1 := New()
	converter1.Add(NewStruct(TestCustomType{}).WithFieldOpts(time.Time{}, TypeOptions{PYType: "Date", PYTransform: "new Date(__VALUE__)"}))
	converter1.BackupDir = ""

	// Test with custom field options defined globally:
	converter2 := New()
	converter2.Add(reflect.TypeOf(TestCustomType{}))
	converter2.ManageType(time.Time{}, TypeOptions{PYType: "Date", PYTransform: "new Date(__VALUE__)"})
	converter2.BackupDir = ""

	for _, converter := range []*Pythony{converter1, converter2} {
		desiredResult := `export class TestCustomType {
	time: Date;
