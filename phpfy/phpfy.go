...
const (
	phpTransformTag      = "php_transform"
	phpType              = "php_type"
	phpConvertValuesFunc = "public function convertValues($any);"
	_                    = `convertValues(a: any, classs: any, asMap: boolean = false): any {
	if (!a) {
		return a;
	}
	if (a.slice) {
...
func (ts Phpfy) logf(depth int, s string, args ...interface{}) {
...
func (t *Phpfy) ManageType(fld interface{}, opts TypeOptions) *Phpfy {
	var typ reflect.Type
	switch t := fld.(type) {
	case reflect.Type:
		typ = t
...
func (t *Phpfy) WithCreateFromMethod(b bool) *Phpfy {
	t.CreateFromMethod = b
	return t
}

func (t *Phpfy) WithInterface(b bool) *Phpfy {
	t.CreateInterface = b
	return t
}

func (t *Phpfy) WithConstructor(b bool) *Phpfy {
	t.CreateConstructor = b
	return t
}

func (t *Phpfy) WithIndent(i string) *Phpfy {
	t.Indent = i
	return t
}

func (t *Phpfy) WithBackupDir(b string) *Phpfy {
	t.BackupDir = b
	return t
}

func (t *Phpfy) WithPrefix(p string) *Phpfy {
	t.Prefix = p
	return t
}

func (t *Phpfy) WithSuffix(s string) *Phpfy {
	t.Suffix = s
	return t
}

func (t *Phpfy) Add(obj interface{}) *Phpfy {
	switch ty := obj.(type) {
	case StructType:
...
func (t *Phpfy) AddType(typeOf reflect.Type) *Phpfy {
...
func (t *typeScriptClassBuilder) AddMapField(fieldName string, field reflect.StructField) {
	...
	t.fields = append(t.fields, fmt.Sprintf("array<%s, %s> %s%s;", keyTypeStr, valueTypeName, t.indent, fieldName))
	if valueType.Kind() == reflect.Struct {
		t.constructorBody = append(t.constructorBody, fmt.Sprintf("%s%s$this->%s = $this->convertValues($source[\"%s\"], %s, true);", t.indent, t.indent, strippedFieldName, strippedFieldName, t.prefix+valueTypeName+t.suffix))
	} else {
		t.constructorBody = append(t.constructorBody, fmt.Sprintf("%s%s$this->%s = $source[\"%s\"];", t.indent, t.indent, strippedFieldName, strippedFieldName))
	}
}

func (t *Phpfy) AddEnum(values interface{}) *Phpfy {
	if t.enums == nil {
...
func (t *Phpfy) AddEnumValues(typeOf reflect.Type, values interface{}) *Phpfy {
	t.AddEnum(values)
	return t
}

func (t *Phpfy) Convert(customCode map[string]string) (string, error) {
...
func (t Phpfy) backup(fileName string) error {
...
func (t Phpfy) ConvertToFile(fileName string) error {
	...
	if _, err := f.WriteString(t.DefaultHeader); err != nil {
		return err
	}
...
func (t *Phpfy) convertEnum(depth int, typeOf reflect.Type, elements []enumElement) (string, error) {
...
func (t *Phpfy) getFieldOptions(structType reflect.Type, field reflect.StructField) TypeOptions {
	// By default use options defined by tags:
	opts := TypeOptions{PHPTransform: field.Tag.Get(phpTransformTag), PHPType: field.Tag.Get(phpType)}
	...
	for _, o := range overrides {
		if o.PHPTransform != "" {
			opts.PHPTransform = o.PHPTransform
		}
		if o.PHPType != "" {
			opts.PHPType = o.PHPType
		}
	}
...
func (t *Phpfy) getJSONFieldName(field reflect.StructField, isPtr bool) string {
	jsonFieldName := ""
...
func (t *Phpfy) convertType(depth int, typeOf reflect.Type, customCode map[string]string) (string, error) {
	if _, found := t.alreadyConverted[typeOf]; found { // Already converted
		return "", nil
	}
	...
	for _, field := range fields {
		...
		} else if fldOpts.PHPType != "" { // Struct:
			t.logf(depth, "- simple field %s.%s", typeOf.Name(), field.Name)
			err = builder.AddSimpleField(jsonFieldName, field, fldOpts)
	...
	if !t.CreateInterface {
		...
		if t.CreateFromMethod {
			result += fmt.Sprintf("\n%sstatic createFrom($source) {\n", t.Indent)
			result += fmt.Sprintf("%s%sreturn new %s($source);\n", t.Indent, t.Indent, entityName)
			result += fmt.Sprintf("%s}\n", t.Indent)
		}
		if t.CreateConstructor {
			result += fmt.Sprintf("\n%spublic function __constructor($source) {\n", t.Indent)
			result += t.Indent + t.Indent + "if ('string' == gettype($source)) $source = json_decode($source);\n"
			result += constructorBody + "\n"
			result += fmt.Sprintf("%s}\n", t.Indent)
		}
		if needsConvertValue && (t.CreateConstructor || t.CreateFromMethod) {
			result += "\n" + indentLines(strings.ReplaceAll(phpConvertValuesFunc, "\t", t.Indent), 1) + "\n"
		}
	}
...
func (t *Phpfy) AddImport(i string) {
	for _, cimport := range t.customImports {
		if cimport == i {
			return
		}
...
func (t *typeScriptClassBuilder) AddSimpleArrayField(fieldName string, field reflect.StructField, arrayDepth int, opts TypeOptions) error {
	...
	if len(fieldName) > 0 {
		strippedFieldName := strings.ReplaceAll(fieldName, "?", "")
		if len(opts.PHPType) > 0 {
			t.addField(fieldName, opts.PHPType)
...
func (t *typeScriptClassBuilder) addField(fld, fldType string) {
	t.fields = append(t.fields, fmt.Sprint(t.indent, "public ", fldType, t.indent, " $", fld, ";"))
}
